...
/* Definitions for QueueSeriale */
osMessageQueueId_t QueueSerialeHandle;
const osMessageQueueAttr_t QueueSeriale_attributes = {
  .name = "QueueSeriale"
};
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void CharToMorse(char input, char *morseCode)
{
	if (input >= 'A' && input <= 'Z')
	{
		strcpy(morseCode, morseAlphabet[input - 'A']);
	}
	else if (input >= '0' && input <= '9')
	{
		strcpy(morseCode, morseAlphabet[input - '0' + 26]);
	}
  else if(input == ' ')
  {
    strcpy(morseCode, ' ');
...
/* Configure the peripherals common clocks */
  ...
  QueueLEDHandle = osMessageQueueNew (16, sizeof(uint8_t), &QueueLED_attributes);

  /* creation of QueueSeriale */
  QueueSerialeHandle = osMessageQueueNew (16, sizeof(uint8_t), &QueueSeriale_attributes);
 ...
 osDelay(3000);
  ...
  for(;;)
  {
   osSemaphoreAcquire(myCountingSem01Handle, osWaitForever);
    osSemaphoreAcquire(myCountingSem01Handle, osWaitForever);//aspetto che i due task "slave" abbiano finito per ripartire"
    tmp_char = usbserialReadChar();//leggo i caratteri da seriale

    if((tmp_char >= 'A' && tmp_char <= 'Z') || (tmp_char >= '0' && tmp_char <= '9')|| (tmp_char == ' '))
    {
    	CharToMorse(tmp_char, morseCode);

    	while(morseCode[i] != '\0') //continuo a leggere la stringa finchè non arrivo a end of line
    	{
    		//osMessageQueuePut(QueueSerialeHandle, &morseCode[i],1, osWaitForever); //commentato per problema con la seriale
    		osMessageQueuePut(QueueLEDHandle, &morseCode[i], 1, osWaitForever);
    		i++;
    	}
    	i = 0;
    }
    ...
    {
    	CharToMorse(' ', morseCode);//messaggio di errore per caratteri non ammessi
    }
  ...
  /* USER CODE BEGIN StartTask04 */
	uint8_t size_coda;
	  ...
	  for(;;)
	  {
		  ...
		  osSemaphoreAcquire(myBinarySem01Handle, osWaitForever); //acquisisco il semaforo
		  size_coda = (uint8_t)osMessageQueueGetCount(QueueLEDHandle);
		  for(i = 0; i < size_coda; i++)
		  {
			osMessageQueueGet(QueueLEDHandle, &char_read, 0, osWaitForever);
			if(char_read == '.')//toggle del led per il punto
			{
			...
			else if(char_read == '-') //toggle del led per la linea
			{
				HAL_GPIO_WritePin(GPIOC, LED0_Pin, GPIO_PIN_SET);
				HAL_Delay(3*period);
				HAL_GPIO_WritePin(GPIOC, LED0_Pin, GPIO_PIN_RESET);
			...
			else if(char_read == ' ') //toggle del led per lo spazio tra parole
			{
				HAL_Delay(2*period);
			}
		  }
		  osSemaphoreRelease(myCountingSem01Handle); //rilascio il semaforo per la prima volta
	...
	uint8_t size_coda;
	  ...
	  for(;;)
	  {
	    ...
	    HAL_GPIO_TogglePin(GPIOC, LED2_Pin);
		  /*   teniamo commentata questa parte di codice perchè la seriale in trasmissione non funziona e blocca l'esecuzione di tutto il programma
		   * il toggle del led è messo comunque per far vedere che il task funziona ed entra.
		   * questa parte di codice dovrebbe solo leggere dalla queue il carattere convertito e stamparlo interamente a video
		   * utilizziamo un for con numero di cicli variabile perchè i caratteri morse hanno lunghezza variabile
		  size_coda = (uint8_t)osMessageQueueGetCount(QueueSerialeHandle);
	    //HAL_GPIO_TogglePin(GPIOC, LED2_Pin);
	    for(i = 0; i < size_coda; i++)
	    {

	    	osMessageQueueGet(QueueSerialeHandle, &char_read, 0, osWaitForever);
	    	if(char_read == ' ')
	    	{
	    		usbserialWriteChar(' '); //metto 3 spazi tra due parole diverse
	    		usbserialWriteChar(' ');
	    	}
	    	else
	    	{
	    		usbserialWriteChar(char_read);
	    	}
	    }
	    usbserialWriteChar(' '); //metto uno spazio dopo ogni lettera di una parola
		*/
